...
type ServerlessEndpointsServer struct{
	...
	// GetStatus is the fake for method ServerlessEndpointsClient.GetStatus
	// HTTP status codes to indicate success: http.StatusOK
	GetStatus func(ctx context.Context, resourceGroupName string, workspaceName string, name string, options *armmachinelearning.ServerlessEndpointsClientGetStatusOptions) (resp azfake.Responder[armmachinelearning.ServerlessEndpointsClientGetStatusResponse], errResp azfake.ErrorResponder)
...
func (s *ServerlessEndpointsServerTransport) Do(req *http.Request) (*http.Response, error) {
	...
	case "ServerlessEndpointsClient.GetStatus":
		resp, err = s.dispatchGetStatus(req)
...
func (s *ServerlessEndpointsServerTransport) dispatchGetStatus(req *http.Request) (*http.Response, error) {
	if s.srv.GetStatus == nil {
		return nil, &nonRetriableError{errors.New("fake for method GetStatus not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.MachineLearningServices/workspaces/(?P<workspaceName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/serverlessEndpoints/(?P<name>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/getStatus`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if matches == nil || len(matches) < 4 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	...
	respr, errRespr := s.srv.GetStatus(req.Context(), resourceGroupNameParam, workspaceNameParam, nameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).ServerlessEndpointStatus, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
